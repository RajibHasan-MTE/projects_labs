...
const float ACS_SENSITIVITY_mV_per_A = 100.0; // mV per Amp for ACS712-20A
...
float currentThreshold = 0.08; // below this consider zero (A)
...
void measureAll(float &Vrms, float &Irms1, float &Irms2, float &Irms3,
  ...
  for (int i = 0; i < SAMPLE_COUNT; i++) {
    ...
    double I1_inst = ((i1_center * (ADC_REF_V / ADC_RES)) * 1000.0) / ACS_SENSITIVITY_mV_per_A; // Amp
    double I2_inst = ((i2_center * (ADC_REF_V / ADC_RES)) * 1000.0) / ACS_SENSITIVITY_mV_per_A;
    double I3_inst = ((i3_center * (ADC_REF_V / ADC_RES)) * 1000.0) / ACS_SENSITIVITY_mV_per_A;
  ...
  double Irms1_converted = (sqrt(meanI1sq_raw) * (ADC_REF_V / ADC_RES) * 1000.0) / ACS_SENSITIVITY_mV_per_A;
  double Irms2_converted = (sqrt(meanI2sq_raw) * (ADC_REF_V / ADC_RES) * 1000.0) / ACS_SENSITIVITY_mV_per_A;
  double Irms3_converted = (sqrt(meanI3sq_raw) * (ADC_REF_V / ADC_RES) * 1000.0) / ACS_SENSITIVITY_mV_per_A;
